...
- **목적**: 로그인한 회원만 PDF 다운로드 가능하도록 제한하여 회원가입 유도
- **현재 상황**: 비로그인 사용자도 PDF 다운로드 버튼 접근 가능
- **구현 계획**:
  1. **UI 조건부 렌더링**:
     - 로그인 사용자: PDF 다운로드 버튼 표시
     - 비로그인 사용자: "로그인 후 이용 가능" 메시지 또는 로그인 유도 버튼
  2. **권한 확인 로직**:
     - `LiveCalculation.tsx`의 PDF 다운로드 함수 수정
     - `ResultSummary.tsx`의 PDF 다운로드 함수 수정
     - 사용자 상태 검증 로직 추가
  3. **로그인 유도 팝업 구현**:
     - `LoginRequiredModal.tsx` 컴포넌트 생성
     - PDF 다운로드 시도 시 팝업 표시
     - 회원가입 혜택 재안내 (5000P, PDF 무제한 이용)
     - 직접 로그인/회원가입 모달 연동
- **사용자 플로우**:
  1. 비로그인 사용자가 PDF 다운로드 버튼 클릭
  2. "로그인이 필요합니다" 팝업 표시
  3. 로그인/회원가입 버튼으로 인증 모달 연결
  4. 로그인 완료 후 PDF 다운로드 자동 진행
